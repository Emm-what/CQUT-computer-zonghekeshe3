...
class MedicalGraph:
    def __init__(self):
        ...
        # 打开安全验证信道
        self.g = Graph('http://localhost:7474/', auth=("1457710323@qq.com", "Yuan990904"))
    '''读取json文件'''
    def read_nodes(self):
        # ７类节点
        drugs = []  # 药品
    ...
    def export_data(self):
        ...
        rels_noteat, rels_doeat, rels_department, rels_commonddrug, rels_drug_producer, rels_recommanddrug, \
        rels_symptom, rels_acompany, rels_category = self.read_nodes()
        f_drug = open('导出数据/drug.txt', 'w+')
        f_food = open('导出数据/food.txt', 'w+')
        f_check = open('导出数据/check.txt', 'w+')
        f_department = open('导出数据/department.txt', 'w+')
        f_producer = open('导出数据/producer.txt', 'w+')
        f_symptom = open('导出数据/symptoms.txt', 'w+')
        f_disease = open('导出数据/disease.txt', 'w+')
